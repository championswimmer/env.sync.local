...
# check sync status
env-sync status</code></pre></section><section class="panel"><h2>Automate sync</h2><pre><code># install cron job (syncs every 30 minutes)
env-sync cron --install

# auto-load secrets in your shell
# add to ~/.bashrc or ~/.zshrc:
eval &quot;$(env-sync load 2&gt;/dev/null)&quot;</code></pre><p>Make sure <code>env-sync serve -d</code> starts at boot so the mTLS server is always available for incoming sync requests. Consider using your OS service manager:</p><pre><code># Linux (systemd)
...
# remove the binary
sudo rm -f /usr/local/bin/env-sync</code></pre><p><strong>Important</strong>: After revoking a device, rotate any secrets it had access to. The revoked device may still have a cached copy of previously-synced secrets.</p></section>`,8)),n("section",h,[e[3]||(e[3]=n("h2",null,"Secure mesh ready",-1)),e[4]||(e[4]=n("p",null,"Your peers are now syncing secrets with end-to-end encryption and mutual authentication.",-1)),n("div",y,[s(t,{class:"btn btn-primary",to:"/usage"},{default:r(()=>[...e[1]||(e[1]=[o("Usage guide →",-1)])]),_:1}),s(t,{class:"btn btn-secondary",to:"/security"},{default:r(()=>[...e[2]||(e[2]=[o("Security deep-dive",-1)])]),_:1})])])],64)}}});export{w as default};
